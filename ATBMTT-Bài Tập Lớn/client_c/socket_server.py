...
HOST = '127.0.0.1'
PORT = 9002  # Node C dùng cổng 9002
...
def handle_client(conn):
    try:
        ...
        if request.get("type") == "download_request":
            print("📩 Node C nhận yêu cầu tải lại file")
            ...
            print(f"✅ Node C đã gửi lại file: {filename}")

        else:
            # Nhận file mới từ người gửi
            metadata = request["metadata"]
            ...
            private_key = load_private_key("keys/private_c.pem")  # 🔐 Node C dùng private_c
            ...
            conn.sendall(b"ACK")
            print(f"✅ Node C đã lưu file: {filename}")

    except Exception as e:
        print(f"❌ Lỗi xử lý tại Node C: {e}")
        try:
            conn.sendall(b"NACK")
        except:
            pass
...
def start_server():
    print(f"🔌 Node C đang lắng nghe tại {HOST}:{PORT}")
